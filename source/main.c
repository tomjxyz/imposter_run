...
void drawCrewmate(Point pos, u32 colour, bool dead) {
  ...
  if (!dead) {
    ...
    // Eye
    GRRLIB_Rectangle(pos.x + 5, pos.y + 8, bodyW, scrHeight / 16, EYES_COLOUR,
                     true);
    // Leg gap
    ...
    // Bone
    GRRLIB_Rectangle(pos.x + (bodyW / 2) - 4, pos.y + (bodyH / 2) - 20, 8, 20,
                     0xFFFFFFFF, true);
    GRRLIB_Circle(pos.x + (bodyW / 2), pos.y + (bodyH / 2) - 20, 8, 0xFFFFFFFF,
                  true);
    // Leg gap
    GRRLIB_Rectangle(pos.x + (bodyW / 2) - 2, pos.y + ((bodyH / 2) + 20), 5, 20,
                     0x000000FF, true);
  }
}
...
void gameplay() {
  ...
  if (imposterPos.y + scrHeight / 6 >= (scrHeight / 6) * 5) {
    if (WPAD_ButtonsHeld(0) & WPAD_BUTTON_A && !dead)
      velocity =
          -JUMP_SPEED; // Add upwards velocity when A pressed and still alive
    else
      velocity = 0;
